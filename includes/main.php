<?php
/**
 * Church Sermon Manager — main body. Loaded from sermons.php after the
 * legacy-coexistence guard passes; nothing here is compiled when it trips.
 *
 * @package SM/Core
 */
...
// Load autoloader
require_once dirname( CSM_PLUGIN_FILE ) . '/autoload.php';

// Initialize the plugin
add_action( 'plugins_loaded', function() {
    // Initialize core plugin
    \SM\Core\Plugin::get_instance();
    
    // Initialize post types
    \SM\Models\PostTypes::get_instance();
    
    // Initialize shortcodes
    \SM\Frontend\Shortcodes::get_instance();
} );
...
class SermonManager { // phpcs:ignore
	...
	public function __construct() {
		...
		define( 'SM_PLUGIN_FILE', CSM_PLUGIN_FILE );
		define( 'SM_PATH', dirname( SM_PLUGIN_FILE ) . '/' );
		define( 'SM_BASENAME', plugin_basename( CSM_PLUGIN_FILE ) );
		define( 'SM_URL', plugin_dir_url( CSM_PLUGIN_FILE ) );
		define( 'SM_VERSION', preg_match( '/^.*Version: (.*)$/m', file_get_contents( CSM_PLUGIN_FILE ), $version ) ? trim( $version[1] ) : 'N/A' );
...
// Note: the old edit_form_after_title hook that wrote the post back on every
// editor load was removed; the Pro templating module supplies the corrected
// editor behavior (meta display with post_content fallback, no DB writes).

// ============================================================
// Pro feature modules (templating, podcasting, builder widgets)
// ============================================================
define( 'SMP_VERSION', SM_VERSION );
define( 'SMP_PATH', SM_PATH . 'pro/' );
define( 'SMP_PATH_INCLUDES', SMP_PATH . 'includes/' );
define( 'SMP_PATH_SHORTCODES', SMP_PATH_INCLUDES . 'shortcodes/' );
define( 'SMP_URL', SM_URL . 'pro/' );
define( 'SMP_ASSETS_URL', SMP_URL . 'assets/' );

add_action( 'plugins_loaded', function () {
	require_once SMP_PATH_INCLUDES . 'plugin.php';
} );

// Some editors submit literal nulls for empty content; normalize so inserts
// never fail on NOT NULL columns.
