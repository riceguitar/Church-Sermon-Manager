...
class Sermon_Blog extends ET_Builder_Module {
	...
	public function render( $unprocessed_props, $content = null, $render_slug = '' ) {
		...
		if ( 'on' == $show_filters ) {
			 ...
			 */
			echo apply_filters( 'smp/shortcodes/divi/sermon_filtering', $content ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- filter form HTML generated by render_wpfc_sorting().
		...
		if ( have_posts() ) {
			while ( have_posts() ) {
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class( $post_classes ); ?>
					<?php echo 'on' === $show_grid ? 'style="margin-right:' . esc_attr( $spacing_columns ) . 'px;width: calc((100% - ' . esc_attr( $spacing_columns * ( $grid_columns - 1 ) ) . 'px) / ' . esc_attr( $grid_columns ) . ');"' : ''; ?>>
					<div class="wpfc-sermon-inner">
						<?php if ( 'on' == $show_image ) : ?>
							<?php if ( 'image' == $featured_type ) : ?>
								<div class="wpfc-sermon-image">
									<a href="<?php the_permalink(); ?>">
										<img src="<?php echo esc_url( get_sermon_image_url( true ) ); ?>"
												alt="<?php the_title(); ?>">
										<div class="wpfc-sermon-image-img" <?php echo 'style="background-image: url(' . esc_url( get_sermon_image_url( true ) ) . ')"'; ?>>
										</div>
									</a>
								</div>
							<?php endif; ?>
							<?php if ( 'video' == $featured_type ) : ?>
								<?php if ( get_wpfc_sermon_meta( 'sermon_video_link' ) ) : ?>
									<div class="wpfc-sermon-video wpfc-sermon-video-link">
										<?php echo wpfc_render_video( get_wpfc_sermon_meta( 'sermon_video_link' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- video player HTML built by wpfc_render_video(). ?>
									</div>
								<?php endif; ?>
						...
						<div class="wpfc-sermon-main <?php echo get_sermon_image_url() ? '' : 'no-image'; ?>">
							<div class="wpfc-sermon-header">
								<div class="wpfc-sermon-header-main">
									...
									<?php if ( 'on' == $show_title ) : ?>
										<h3 class="wpfc-sermon-title" <?php echo 'style="padding-bottom:' . esc_attr( $title_padding ) . 'px;"'; ?>>
									...
									<?php if ( 'on' == $show_date ) : ?>
										<div class="wpfc-sermon-meta-item wpfc-sermon-meta-date">
											...
											<?php else : ?>
												<?php echo esc_html( \SM_Dates::get( $meta_date ) ); ?>
											<?php endif; ?>
										</div>
									<?php endif; ?>
								</div>
							...
							<div class="wpfc-sermon-description">
								<?php if ( 'on' == $show_excerpt ) : ?>
									<div class="sermon-description-content" <?php echo 'style="padding-bottom:' . esc_attr( $description_padding ) . 'px;"'; ?>>
										<?php if ( has_excerpt() ) : ?>
											<?php echo get_the_excerpt(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- excerpt HTML already filtered by WordPress via get_the_excerpt(). ?>
										<?php else : ?>
											<?php echo esc_html( wp_trim_words( get_wpfc_sermon_meta( 'sermon_description' ), $excerpt_length ) ); ?>
										<?php endif; ?>
									</div>
									<?php if ( ( str_word_count( get_wpfc_sermon_meta( 'sermon_description' ) ) > $excerpt_length ) && ( 'on' == $show_readmore ) ) : ?>
										<div class="wpfc-sermon-description-read-more">
											<a href="<?php echo esc_url( get_permalink() ); ?>"><?php echo esc_html( $read_more_text ); ?></a>
										</div>
									<?php endif; ?>
								<?php endif; ?>
							</div>
							<?php if ( ( 'on' == $show_sermon_audio ) && ( get_wpfc_sermon_meta( 'sermon_audio' ) || get_wpfc_sermon_meta( 'sermon_audio_id' ) ) ) : ?>
								...
								<div class="wpfc-sermon-audio">
									<?php echo wpfc_render_audio( $sermon_audio_url ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- audio player HTML built by wpfc_render_audio(). ?>
								</div>
							<?php endif; ?>
								...
								<div class="wpfc-sermon-footer">
									<?php if ( ( 'on' == $show_preacher ) && ( has_term( '', 'wpfc_preacher', get_the_ID() ) ) ) : ?>
										<div class="wpfc-sermon-meta-item wpfc-sermon-meta-preacher">
											...
											?>
											<span class="wpfc-sermon-meta-prefix"><?php echo esc_html__( 'Preacher', 'church-sermon-manager' ); ?>
												:</span>
									...
									<?php if ( ( 'on' == $show_passage ) && ( get_wpfc_sermon_meta( 'bible_passage' ) ) ) : ?>
										<div class="wpfc-sermon-meta-item wpfc-sermon-meta-passage">
											<span class="wpfc-sermon-meta-prefix"><?php echo esc_html__( 'Passage', 'church-sermon-manager' ); ?>
												:</span>
									...
									<?php if ( ( 'on' == $show_service_type ) && ( has_term( '', 'wpfc_service_type', get_the_ID() ) ) ) : ?>
										<div class="wpfc-sermon-meta-item wpfc-sermon-meta-service">
											<span class="wpfc-sermon-meta-prefix"><?php echo esc_html__( 'Service Type', 'church-sermon-manager' ); ?>
												:</span>
			...
			if ( ( $paginate_links ) && ( 'on' == $show_pagination ) ) {
				?>
				<nav class="custom-pagination" style="text-align:<?php echo esc_attr( $pagination_alignment ); ?>;">
					<p><?php echo $paginate_links; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- pagination markup generated by paginate_links(). ?></p>
				</nav>
				<?php
			}
		}
